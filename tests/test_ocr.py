...
def test_tesseract():
    ...
    try:
        ...
        print(f"\nâœ… Tesseract OCR is installed and working!")
        ...
        try:
            ...
            print(f"\nâœ… PIL (Pillow) is installed")
        except ImportError:
            print(f"\nâš ï¸  PIL (Pillow) not installed - install with: pip install Pillow")
        
        try:
            from pdf2image import convert_from_path
            print(f"âœ… pdf2image is installed")
        except ImportError:
            print(f"âš ï¸  pdf2image not installed - install with: pip install pdf2image")
        
        print("\n" + "=" * 70)
        print("TESSERACT OCR: READY FOR USE âœ…")
        print("=" * 70)
        
        logger.info("Tesseract OCR test completed successfully")
        
    except Exception as e:
        print(f"\nâŒ Error testing Tesseract: {e}")
